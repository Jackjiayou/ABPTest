...
namespace ABPCoreSpa.Tests
{
    public abstract class ABPCoreSpaTestBase : AbpIntegratedTestBase<ABPCoreSpaTestModule>
    {
        ...
        protected void UsingDbContext(Action<ABPCoreSpaDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ABPCoreSpaDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ABPCoreSpaDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ABPCoreSpaDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ABPCoreSpaDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ABPCoreSpaDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ABPCoreSpaDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ABPCoreSpaDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ABPCoreSpaDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ABPCoreSpaDbContext, Task<T>> func)
        {
            T result;
